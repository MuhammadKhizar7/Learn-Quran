  ...
  <v-container grid-list-md text-xs-center>
      ...
      <v-flex md9 xs12>
        <div dir="rtl" lang="ar">
          <v-card color="white" class>
            ...
            <ul>
              <li :class="currentAayah(index)" v-for="(ayah,index) in ayahs" :key="index">
                <span>{{ayah.text}}</span>
    ...
    <div v-if="isLoaded">
      <div id="audio" class="player-wrapper">
        <audio-player :file="currentAudio.src"></audio-player>
      </div>
    </div>
  </v-container>
  </div>
...
export default {
  ...
  methods: {
     currentAayah: function(index){
      return index===0? "activeAyah":"";
    },
    next(page) {
        ...
        .then(response => {
          ...
          response.data.data.ayahs.forEach(element => {
            this.audioQuran.push({
              ...
              src:
                "http://cdn.alquran.cloud/media/audio/ayah/ar.abdulsamad/" +
                element.number
            });
          });
          // this.isLoaded=true
